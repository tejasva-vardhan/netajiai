...
- **ACTION REQUIRED — Web/platform/security:** deploy `apps/web` with `NEXT_PUBLIC_API_BASE_URL` pointing to the HTTPS greenfield API, register separate public OIDC callbacks for `/admin/auth/callback`, `/auth/callback`, and `/auth/silent-callback`, set `NEXT_PUBLIC_OIDC_ISSUER`, `NEXT_PUBLIC_OIDC_CLIENT_ID`, citizen redirect variables, and approved scopes, add the web origin to backend CORS, and map provider `roles` to backend capabilities. Approve the capability matrix, bearer-token session/logout policy, browser privacy policy, and staging 401/403/expiry tests before enabling privileged access. No OIDC client secret may be shipped to the browser.
